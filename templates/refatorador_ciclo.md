...
6. Certifique-se de que todos os axiomas/princípios na base resultante `principios_canonicos.md` possuam um bloco de metadados YAML/frontmatter contendo a escala de incerteza com a chave `confianca` assumindo um dos seguintes valores: `alto`, `medio`, `baixo` ou `especulativo`.

[OUTPUT]
Sua resposta deve conter duas seções principais separadas exatamente por `=== DIVIDER ===`:

1. Novo conteúdo para `principios_canonicos.md` (onde cada axioma/princípio tem seu respectivo frontmatter YAML com a chave `confianca`)
=== DIVIDER ===
